...
// Icon mapping for submenu items based on href
const SUBMENU_ICONS: Record<string, LucideIcon> = {
  // About submenus
  '/about/vision-mission': Target,
  '/about/trust': Heart,
  '/about/management': Users,
  '/about/institutions': Building2,
  '/about/principal-message': MessageSquare,
  '/about/ncte-approval': FileCheck,

  // Departments submenus
  '/departments/tamil': Languages,
  '/departments/english': BookA,
  '/departments/zoology': Bug,
  '/departments/maths': Calculator,
  '/departments/chemistry': FlaskConical,
  '/departments/computer-science': Laptop,
  '/departments/commerce': TrendingUp,
  '/departments/physics': Zap,
  '/departments/botany': Leaf,
  '/departments/history': Clock,
  '/departments/economics': BarChart3,
  '/departments/political-science': Scale,
  '/departments/social-science': Globe,
  '/departments/microbiology': Microscope,

  // Facilities submenus
  '/facilities/library': BookOpen,
  '/facilities/hostel': Hotel,
  '/facilities/class-room': School,
  '/facilities/auditorium': Music,
  '/facilities/transport': Bus,
  '/facilities/wifi': Wifi,
  '/facilities/ambulance-services': Ambulance,
  '/facilities/food-court': Utensils,
  '/facilities/bank-post-office': Landmark,
  '/facilities/hospital': Hospital,
  '/facilities/seminar-hall': Presentation,

  // Others submenus
  '/others/biomatric-list': Fingerprint,
  '/others/balance-sheet': FileSpreadsheet,
  '/others/financial-details': Wallet,
  '/others/digital-campus': Smartphone,
  '/others/faculty-details': Users,
  '/others/careers': Briefcase,
  '/others/alumni': GraduationCap,
  '/others/privacy-policy': Shield
};

// Routes that are parent-only (no actual page, only submenus)
const PARENT_ONLY_ROUTES = new Set<string>([]);
      ...
      menu.submenus.forEach((sub) => {
        ...
        if (!seenHrefs.has(subHref)) {
          ...
          items.push({
            href: subHref,
            label: sub.label,
            icon: SUBMENU_ICONS[subHref] || menu.icon,
            active: sub.active
          });
        }
      });
